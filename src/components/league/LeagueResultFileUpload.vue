  ...
  <div class="example-drag">
    ...
    <template v-if="files.length">
      ...
      <v-btn class="mt-5" color="info" @click="resultFileUpload()">
        <v-icon style="margin-right:5px;">fas fa-arrow-circle-up</v-icon>리그
        결과 등록
      </v-btn>
    </template>
    <template v-else>
      ...
      <v-row>
        <v-col cols="12" sm="12" md="12">
          <div class="text-center p-5 black--text">
            <p
              >등록하실 리그 리플레이 파일을 이곳으로 드래그 앤 드롭
...
export default {
  ...
  methods: {
    resultFileUpload() {
      ...
      if (this.schedule.idx == null) {
        alert("진행하신 경기를 선택해주세요.");
        return false;
      }
    ...
    getFormData() {
      ...
      formData.append("leagueIndex", this.leagueIndex);
      formData.append("scheduleIdx", this.schedule.idx);
